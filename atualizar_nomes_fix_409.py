...
# -*- coding: utf-8 -*-
"""
Aplicar algoritmo SUPER INTELIGENTE - FIX para erro 409
Agora enviando o email também no payload
"""
...
print("="*80)
print("ATUALIZAÇÃO INTELIGENTE DE NOMES NO CRM (FIX 409)")
print("="*80)
print()
...
for i, lead in enumerate(leads_para_atualizar, 1):
    ...
    # ENVIAR EMAIL NO PAYLOAD para evitar erro 409
    payload = {
        'name': lead['nome_novo'],
        'companyName': lead['nome_novo'],
        'email': lead['email']  # FIX: Incluir email no payload
    }

    # Incluir phone e whatsApp se disponíveis
    if lead.get('phone'):
        payload['phone'] = lead['phone']
    if lead.get('whatsApp'):
        payload['whatsApp'] = lead['whatsApp']
